            ...
            [] as CombinedFeatureFlagAndOverrideType[],
            {
                ...
                setOverriddenUserFlag: async ({ flagId, overrideValue }, breakpoint) => {
                    const response = await toolbarFetch(
                        `api/projects/${teamLogic.values.currentTeam?.id}/feature_flag_overrides/my_overrides`,
                        'POST',
                        {
                            feature_flag: flagId,
                ...
                deleteOverriddenUserFlag: async ({ overrideId }, breakpoint) => {
                    const response = await toolbarFetch(
                        `api/projects/${teamLogic.values.currentTeam?.id}/feature_flag_overrides/${overrideId}`,
                        'DELETE'
                    )
                    breakpoint()
                    if (!response.ok) {
