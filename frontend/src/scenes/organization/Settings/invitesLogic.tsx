...
export const invitesLogic = kea<invitesLogicType>({
    loaders: ({ values }) => ({
        invites: {
            ...
            loadInvites: async () => {
                return (await api.get(`api/organizations/${organizationLogic.values.currentOrganization?.id}/invites/`))
                    .results
            },
            createInvite: async ({ targetEmail }: { targetEmail?: string }) => {
                const newInvite: OrganizationInviteType = await api.create(
                    `api/organizations/${organizationLogic.values.currentOrganization?.id}/invites/`,
                    {
            ...
            deleteInvite: async (invite: OrganizationInviteType) => {
                await api.delete(
                    `api/organizations/${organizationLogic.values.currentOrganization?.id}/invites/${invite.id}/`
                )
                preflightLogic.actions.loadPreflight() // Make sure licensed_users_available is updated
                toast(
